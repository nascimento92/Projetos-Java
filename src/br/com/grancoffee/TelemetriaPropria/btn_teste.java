...
public class btn_teste implements AcaoRotinaJava {
	...
	public void doAction(ContextoAcao arg0) throws Exception {
		getListaPendente(arg0);
	}

	private void getListaPendente(ContextoAcao arg0) throws Exception {
		EntityFacade dwfEntityFacade = EntityFacadeFactory.getDWFFacade();

		Collection<?> parceiro = dwfEntityFacade.findByDynamicFinder(new FinderWrapper("GCSolicitacoesAbastecimento",
				"this.STATUS=? AND this.REABASTECIMENTO=? AND this.NUMOS IS NULL", new Object[] { "1", "S" }));
					...
					if (compareTo < 0) { // GERAR AGORA
						...
						try {
							...
							if (solicitante.intValue() == 3082) {
								...
								if (erro == "") {
									BigDecimal gerarPedidoENota = gerarPedidoENota(patrimonio, DynamicVO, idretorno, id, nunota, substituto,
											solicitante);
									arg0.setMensagemRetorno(""+gerarPedidoENota);
								} else {
									cancelarVisita(patrimonio, id, erro, idretorno);
								}

							} else {
								System.out.println(" ******** Geração pedido abastecimento. Patrimonio: " + patrimonio
										+ " solicitante: " + solicitante);
								BigDecimal gerarPedidoENota = gerarPedidoENota(patrimonio, DynamicVO, idretorno, id, nunota, substituto, solicitante);
								arg0.setMensagemRetorno(""+gerarPedidoENota);
							}
	...
	private void cancelarVisita(String patrimonio, BigDecimal id, String motivo, BigDecimal idretorno) {
		try {
			EntityFacade dwfEntityFacade = EntityFacadeFactory.getDWFFacade();
			Collection<?> parceiro = dwfEntityFacade.findByDynamicFinder(new FinderWrapper(
					"GCSolicitacoesAbastecimento", "this.CODBEM=? AND this.ID=? ", new Object[] { patrimonio, id }));
			for (Iterator<?> Iterator = parceiro.iterator(); Iterator.hasNext();) {
				PersistentLocalEntity itemEntity = (PersistentLocalEntity) Iterator.next();
			...
			salvarException("[cancelarVisita] Nao foi possivel cancelar a visita! patrimonio: " + patrimonio + " id: "
					+ id + "\n" + e.getMessage() + "\n" + e.getCause());
		}
		...
		try {
			...
			Collection<?> parceiro = dwfEntityFacade
					.findByDynamicFinder(new FinderWrapper("AD_RETABAST", "this.ID=?", new Object[] { idretorno }));
			for (Iterator<?> Iterator = parceiro.iterator(); Iterator.hasNext();) {
				PersistentLocalEntity itemEntity = (PersistentLocalEntity) Iterator.next();
				EntityVO NVO = (EntityVO) ((DynamicVO) itemEntity.getValueObject()).wrapInterface(DynamicVO.class);
				DynamicVO VO = (DynamicVO) NVO;
			...
			// dwfFacade.removeByCriteria(new FinderWrapper("AD_RETABAST", "this.ID=?",new
			// Object[] {idretorno}));

		} catch (Exception e) {
			salvarException("[cancelarVisita] Nao foi possivel excluir o retorno de abastecimento! id retorno: "
					+ idretorno + "\n" + e.getMessage() + "\n" + e.getCause());
		}
	}
	...
	private BigDecimal gerarPedidoENota(String patrimonio, DynamicVO gc_solicitabast, BigDecimal idRetorno,
			BigDecimal idsolicitacao, BigDecimal nto, BigDecimal sub, BigDecimal solicitante) throws Exception {
	...
	private void verificaPlanogramaPendente(String patrimonio, BigDecimal numos, BigDecimal nunota,
		...
		try {
			...
			for (Iterator<?> Iterator = parceiro.iterator(); Iterator.hasNext();) {
				...
				BigDecimal estoque = validaEstoqueDoItem(
						BigDecimalUtil.getValueOrZero(DynamicVO.asBigDecimal("ESTOQUE")));
	...
	private void salvaNumeroOS(BigDecimal numos, String patrimonio, BigDecimal idSolicitacao, BigDecimal idRetorno,
			DynamicVO gc_solicitabast, BigDecimal sub) {
	...
	private void geraItemOS(BigDecimal numos, String patrimonio, DynamicVO gc_solicitabast, BigDecimal sub)
			throws Exception {
	...
	private void identificaItens(BigDecimal nunota, String patrimonio, DynamicVO gc_solicitabast) {
		try {
			...
			// TODO::Verifica o local de abastecimento a partir dos cadastros do
			// endereçamento.
			// 01/06 --inicio
			...
			for (Iterator<?> Iterator = parceiro.iterator(); Iterator.hasNext();) {
				...
				if (falta.doubleValue() <= estoqueNaEmpresa.doubleValue() && falta.doubleValue() > 0) {
					if (qtdMinima.doubleValue() > 1) {
						if (falta.doubleValue() > qtdMinima.intValue()) {
							...
							if (qtdParaNota.doubleValue() <= nivelpar.doubleValue()) {
								sequencia++;
								insereItemNaNota(nunota, empresaAbast, localAbast, produto, volume, qtdParaNota,
								...
								insereItemEmRuptura(nunota, empresaAbast, localAbast, produto, volume, falta,
										new BigDecimal(sequencia), valorTotal, valor, tecla, top, gc_solicitabast,
										patrimonio,
										"Falta " + falta + ", quantidade para ser abastecida " + qtdParaNota
												+ ", nível par " + nivelpar
												+ ", quantidade para a nota superior ao nível par.",
										nivelpar, estoque);
							}
							...
							insereItemEmRuptura(nunota, empresaAbast, localAbast, produto, volume, falta,
									new BigDecimal(sequencia), valorTotal, valor, tecla, top, gc_solicitabast,
									patrimonio, "Produto não atingiu a quantidade mínima de " + qtdMinima + " itens.",
									nivelpar, estoque);
						}
					} else {
						sequencia++;
					...
					insereItemEmRuptura(nunota, empresaAbast, localAbast, produto, volume, falta,
							...
							"Ruptura na filial", nivelpar, estoque);
				}
	...
	private void insereItemNaNota(BigDecimal nunota, BigDecimal empresa, BigDecimal local, BigDecimal produto,
			...
			BigDecimal top, DynamicVO gc_solicitabast) {
		try {
	...
	private BigDecimal geraCabecalho(String patrimonio, DynamicVO gc_solicitabast, BigDecimal solicitante)
			throws Exception {
...
// VALIDAÇÕES
	...
	private String validacoes(String patrimonio, String abastecimento, Timestamp dtagendamento, Timestamp dtatendimento,
			BigDecimal rota, BigDecimal id) {
		...
		try {
			...
			if (verificaSeAhMaquinaPossuiPlanograma(patrimonio)) {
				erro = "Máquina " + patrimonio + " não possui planograma, quantidade de teclas: " + this.qtdTeclas
						+ " hora atual: " + TimeUtils.getNow();
			}

			// TODO:: verifica se existe visita pendente sem ajste. 20/06 retirado a pedido
			// da Vania
			// if(validaSeExisteVisitaSemAjusteReabastecimento(patrimonio)) {
			// erro = erro+"\nMáquina "+patrimonio+" possui visita anterior pendente de
			// ajuste por parte do setor de controladoria";
			// }
			...
			if (validaPedido(patrimonio, abastecimento, id)) {
				...
				// String retorno = "Máquina: "+patrimonio+ " OS pendente: "+numeroOS+" tipo de
	...
	private boolean validaSeAhMaquinaEstaNaRota(String patrimonio) throws Exception {
		...
		try {
			...
			nativeSql.resetSqlBuf();
			nativeSql.appendSql(
					"SELECT CASE WHEN EXISTS(SELECT CODBEM FROM AD_ROTATELINS I JOIN AD_ROTATEL R ON (R.ID=I.ID) WHERE I.CODBEM='"
							+ patrimonio + "' AND R.ROTATELPROPRIA='S') THEN 'S' ELSE 'N' END AS VALIDA FROM DUAL");
			contagem = nativeSql.executeQuery();
			while (contagem.next()) {
	...
	private boolean validaSeOhPedidoDeAbastecimentoPoderaSerGerado(String secosCongelados, String codbem) {
		...
		try {
			...
			nativeSql.appendSql("SELECT COUNT(*) AS QTD " + "FROM("
					...
					+ "WHERE (E.ESTOQUE - E.RESERVADO) >= FALTA AND '" + tipoAbastecimento
					+ "' = AD_CONGELADOS AND TRUNC(FALTA/AD_QTDMIN)>0 " + ")");
			contagem = nativeSql.executeQuery();
			while (contagem.next()) {
				int count = contagem.getInt("QTD");
				if (count >= 1) {
		...
		try {
			...
			if ("1".equals(secosCongelados)) {
				nativeSql.appendSql("SELECT COUNT(*) FROM GC_SOLICITABAST WHERE CODBEM='" + patrimonio
						+ "' AND STATUS IN ('1','2') AND REABASTECIMENTO='S' AND NVL(AD_TIPOPRODUTOS,'1')='1' AND NUMOS IS NOT NULL AND ID <> "
						+ id);
			} else if ("2".equals(secosCongelados)) {
				nativeSql.appendSql("SELECT COUNT(*) FROM GC_SOLICITABAST WHERE CODBEM='" + patrimonio
						+ "' AND STATUS IN ('1','2') AND REABASTECIMENTO='S' AND NVL(AD_TIPOPRODUTOS,'1')='2' AND NUMOS IS NOT NULL AND ID <> "
						+ id);
			} else {
				nativeSql.appendSql("SELECT COUNT(*) FROM GC_SOLICITABAST WHERE CODBEM='" + patrimonio
						+ "' AND STATUS IN ('1','2') AND REABASTECIMENTO='S' AND AD_TIPOPRODUTOS IN ('1','2') AND NUMOS IS NOT NULL AND ID <> "
						+ id);
			}
