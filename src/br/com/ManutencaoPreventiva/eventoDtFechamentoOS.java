...
public class eventoDtFechamentoOS implements EventoProgramavelJava {
	/**
	 * 04/10/19 15:42 - Criação do Objeto insetido na TCSOSE toda OS que for
	 * preventiva e for encerrada, irá alimentar os campos de fechamento na tela
	 * controle de man. prev. 21/10/19 17:32 - Inserida a funcionalidade de se a OS
	 * for uma man. prev. programada ou man. prev ele irá atualizar as informações
	 * na tela controle de manutenção preventiva, isso acontece pq o sac também abre
	 * OS de man preventivas.
	 */
		...
		if ("F".equals(situacao) && statusOS != null) {
			...
			if ("N".equals(manPreventiva) || manPreventiva == null) {
				...
				try {
					...
					validaSeEhUmaOsDeManutencaoPreventiva(numos, dataAbertura, usuarioResponsavel, codparc, contrato,
							statusOS);
	...
	private void atualizaInformacao(BigDecimal numos, Timestamp dataFechamento, BigDecimal cancelada) throws Exception {
		...
		Collection<?> parceiro = dwfEntityFacade
				.findByDynamicFinder(new FinderWrapper("AD_MANUPREVOS", "this.NUMOS=?", new Object[] { numos }));
		...
		Collection<?> b = dwfEntityFacade
				.findByDynamicFinder(new FinderWrapper("AD_MANUPREV", "this.ULTIMAOS=?", new Object[] { numos }));
	...
	private void validaSeEhUmaOsDeManutencaoPreventiva(BigDecimal numos, Timestamp dataAbertura,
			BigDecimal usuarioResponsavel, BigDecimal codparc, BigDecimal contrato, BigDecimal statusOS)
			throws Exception {
		...
		try {
			...
			Collection<?> parceiro = dwfEntityFacade.findByDynamicFinder(
					new FinderWrapper("ItemOrdemServico", "this.NUMOS = ? ", new Object[] { numos }));
				...
				if (motivo.equals(new BigDecimal(14))) {
					...
					if (patrimonio != null) {
						...
						if (validaSeExisteNaTelaControleDeManutencaoPreventiva(patrimonio)) {
							atualizaInformacaoPreventivas(numos, patrimonio, dataAbertura, usuarioResponsavel, codparc,
									contrato, statusOS);
						} else {
							cadastraNaTelaControleManutencaoPreventiva(numos, patrimonio, dataAbertura,
									usuarioResponsavel, codparc, contrato);
						}
					}
				}
			}
	...
	private void cadastraNaTelaControleManutencaoPreventiva(BigDecimal numos, String patrimonio, Timestamp dataAbertura,
			BigDecimal usuarioResponsavel, BigDecimal codparc, BigDecimal contrato) throws Exception {
	...
	private void atualizaInformacaoPreventivas(BigDecimal numos, String patrimonio, Timestamp dataAbertura,
			BigDecimal usuarioResponsavel, BigDecimal codparc, BigDecimal contrato, BigDecimal statusOS)
			throws Exception {
